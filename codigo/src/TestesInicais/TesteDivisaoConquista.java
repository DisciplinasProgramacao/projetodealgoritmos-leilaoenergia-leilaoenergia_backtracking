...
public class TesteDivisaoConquista {
    public static void main(String[] args) {
        ...
        try (FileWriter writer = new FileWriter("resultadosDivisaoConquista.csv")) {
            ...
            for (int tamanho = tamanhoInicial; tamanho <= 130 ; tamanho += incremento) {
                ...
                for (int teste = 0; teste < numTestes; teste++) {
                    ...
                    DivisaoConquista solucao = new DivisaoConquista ();

                    long inicioTempo = System.currentTimeMillis();
                    List<Lance> melhorConjunto = solucao.resolver(lances, energia);
                    long fimTempo = System.currentTimeMillis();

                    long duracao = fimTempo - inicioTempo;
                    duracoes.add(duracao);

                    List<Lance> solucaoAtual = melhorConjunto;
                    todasSolucoes.add(solucaoAtual);

                    if (solucao.calcularLucro (solucaoAtual) > maxLucroGeral) {
                        maxLucroGeral = solucao.calcularLucro (solucaoAtual);
